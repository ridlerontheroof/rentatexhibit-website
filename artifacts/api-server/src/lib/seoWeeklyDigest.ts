 ...
 * "https://www.rentatexhibit.com/") if the grant lands elsewhere.
 */
const DEFAULT_GSC_SITE_URL = "sc-domain:rentatexhibit.com";
...
async function fetchBlogUrls(fetchImpl: typeof fetch): Promise<string[]> {
  const res = await fetchImpl(`${SITE}/sitemap.xml`, {
    signal: AbortSignal.timeout(FETCH_TIMEOUT_MS),
    headers: { "User-Agent": "exhibit-seo-digest/1.0" },
  });
  if (res.status !== 200) {
