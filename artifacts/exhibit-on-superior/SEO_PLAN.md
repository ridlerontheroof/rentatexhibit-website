...
- **Resolved (July 26, 2026):** A Squarespace Domain Forwarding rule now issues a real HTTP 301 for the apex: `https://rentatexhibit.com/<path>` → `http://www.rentatexhibit.com/<path>` (301, paths maintained) → `https://www.rentatexhibit.com/<path>` (301). Verified with curl. Setup notes: forwarding could not save while the apex A record lived inside the locked "Domain Connect to Entri" / "Squarespace Domain Connect" DNS presets — both presets had to be deleted first (the `replit-verify` TXT on `@` was re-added as a custom record). Squarespace's forwarder drops query strings; the inline JS redirect in `index.html` stays as a safety net and preserves query + hash for real browsers.
- **Priority:** 4.
