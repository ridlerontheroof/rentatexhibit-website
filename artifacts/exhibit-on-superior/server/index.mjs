...
function serveFile(req, res, filePath, status = 200, urlPathForCache = null) {
  ...
  if (ext === '.md') res.set('X-Robots-Tag', 'noindex, follow');
...
app.use((req, res) => {
  ...
  // Implementation filenames are build details, not public URLs. Redirect
  // them before content negotiation or generic static-file serving so every
  // page has one clean crawl target. True assets (anything other than an
  // index.html implementation file) remain directly available.
  let implementationIndexTarget = null;
  if (urlPath === '/index.html' || urlPath === '/index.html/') {
    implementationIndexTarget = '/';
  } else {
    const match = urlPath.match(/^(.+)\/index\.html\/?$/);
    if (match) implementationIndexTarget = match[1] || '/';
  }
  if (implementationIndexTarget) {
    const qs = req.url.includes('?') ? req.url.slice(req.url.indexOf('?')) : '';
    res.set('Cache-Control', 'public, max-age=3600');
    return res.redirect(301, implementationIndexTarget + qs);
  }
