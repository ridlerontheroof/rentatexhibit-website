...
<html lang="en">
  ...
  <body>
    <p>This page has moved. <a href="${to}">Continue to the property website</a>.</p>
  </body>
</html>
`;
...
// grouped for AI assistants that fetch one file for full site context.
{
  ...
  // PROPERTY CONFIG: provide the property name, address, phone, and email below
  // with the property's verified details before publishing.
  const PROPERTY_DISPLAY_NAME = process.env.PROPERTY_NAME;
  const PROPERTY_TAGLINE = process.env.PROPERTY_TAGLINE;
  if (!PROPERTY_DISPLAY_NAME || !PROPERTY_TAGLINE) {
    throw new Error('PROPERTY_NAME and PROPERTY_TAGLINE are required to generate crawler catalogs');
  }

  const lines = [
    `# ${PROPERTY_DISPLAY_NAME} — Full Page Catalog`,
    '',
    ...
    `> ${PROPERTY_DISPLAY_NAME}: ${PROPERTY_TAGLINE}.`,
    '',
    '## Site Pages',
    '',
    ...indexable.map((p) => {
  ...
  // PROPERTY CONFIG: provide the property description, address, and contact
  const agentsMd = [
    `# AGENTS.md — ${PROPERTY_DISPLAY_NAME}`,
    '',
    `Guidance for AI agents and crawlers reading ${SITE_URL.replace('https://', '')}.`,
    '',
    '## What this site is',
    '',
    // PROPERTY CONFIG: use only verified description, address, and contact details
    `The official leasing site for ${PROPERTY_DISPLAY_NAME}, a ${PROPERTY_TAGLINE}.`,
    `Address: ${process.env.PROPERTY_STREET_ADDRESS || 'Not published'}.`,
    `Phone: ${process.env.PROPERTY_PHONE || 'Not published'}. Email: ${process.env.PROPERTY_EMAIL || 'Not published'}.`,
    '',
