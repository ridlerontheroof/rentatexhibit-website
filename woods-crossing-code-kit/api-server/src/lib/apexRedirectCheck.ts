 ...
 *
 * YOUR-APEX-DOMAIN issues a real HTTP 301 to www via a Squarespace Domain
 ...
 *
 * This module periodically fetches https://YOUR-APEX-DOMAIN/fees without
 * following redirects and alerts when the response is not a 301/308 whose
 * Location points at www.YOUR-DOMAIN.com.
 *
 ...
 * Healthy = 301/308 with a Location whose host is www.YOUR-DOMAIN.com
...
export async function checkApexRedirectOnce(
  ...
  try {
    const res = await fetchImpl(APEX_CHECK_URL, {
      ...
      headers: { "user-agent": "property-site-apex-redirect-check/1.0" },
    });
