...
- A burst of identical `script-src-elem|inline` reports across unrelated pages within seconds can come from one automated or instrumented browser injecting code. Do not add a hash without reproducing the block in clean Chromium and running the enforced `check:csp` guard; if both are clean, keep the policy unchanged and treat the block as CSP doing its job.
