...
- **Sandwich proof for a dropped startup heartbeat:** watchdogs start sequentially in index.ts; if the heartbeats of the watchdogs started immediately before AND after the silent one are visible, the silent one's start line definitely ran — its heartbeat just landed in a still-dropped gap (observed 2026-08-04: a ~10s stdout gap right after "Server listening" ate the apply-link heartbeat while tour-unit at +10s survived). Combine with a manual probe + empty `email_throttle_counters` failure key in the prod DB to conclude healthy.
- Autoscale + frequent publishes mean each fresh instance re-emits its first-check heartbeat inside the drop window; the once-per-UTC-day line is only observable when an instance survives past 00:00 UTC — old log history is not retrievable, so time-boxed "check the logs on day X" verifications expire.
...
- **Dropped startup runs also silently consume once-per-day info gates.** Confirmed after the 2026-08-12 publish: apex/apply-link/showing-scheduler startup passes ran healthy inside the drop window — line eaten AND the createDailyInfoGate day marker consumed, so no "probe passed" info line could appear until the next UTC day. Fix applied: all four (apex, apply-link, showing-scheduler, tour-unit) now use the gtmCheck/redirectCheck STARTUP_DELAY_MS (60s) pattern for the first run.
- **How to apply:** when confirming any new watchdog "reports in after publish", don't expect the started line; every watchdog's first probe must be delayed ≥60s past boot (STARTUP_DELAY_MS pattern) or its startup line and daily info gate are wasted.
