...
- Live units come from the AppFolio Reports API `unit_vacancy.json` (POST, HTTP Basic with APPFOLIO_CLIENT_ID/SECRET secrets) on `highlandptrs.appfolio.com`, proxied by the api-server `/availability` route with a 5-min cache.
