...
**How to apply:** the watcher persists the last build whose required checks and mirror sync both succeeded in the workspace's gitignored cache. Startup compares the live stamp with that marker, so late starts process unseen publishes and restarts skip completed ones. Never advance the marker before every required step succeeds. On failure the watcher exits non-zero so the workflow shows failed — restart it after fixing.
