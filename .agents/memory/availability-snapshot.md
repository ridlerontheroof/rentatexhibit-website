...
- The API server also commits its own baked seed (refreshed non-fatally in its build, same 48h max age): a cold-started autoscale instance seeds its cache from it (marked stale) and the route serves stale-while-revalidate, so the first visitor after a full idle never waits on the AppFolio round-trip. Missing credentials still 503 (checked before the cache).
