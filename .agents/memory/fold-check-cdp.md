...
**Flake + fix:** under validation-run load the Vite dev server can reload mid-check, destroying the CDP execution context ("CDP Execution context was destroyed"). The script's `Cdp.eval` now retries idempotent evals up to 3× on that error — keep that retry if the eval helper is refactored.

**Production-target variant:** check-rented-noindex.mjs reuses the same CDP client but pointed at www.rentatexhibit.com (no local server) — needed because /api/availability is only reachable via production. Poll a head-state snapshot until hydration settles (sold-out title + noindex appear only after the client query resolves); never assert on the first paint, the prerendered stale head renders first.
